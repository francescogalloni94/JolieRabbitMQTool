...
main{
      ...
      fileOL="include \"ini_utils.iol\"\n"
       ...
       fileOL+="init{\n"
       			...
       			+"portRequest.name.name=iniResponse.fileParameter.port_name;\n"
       			+"portRequest.filename=iniResponse.fileParameter.file_name;\n"
       			+"getInputPortMetaData@MetaJolie(portRequest)(portres);\n"
       			+"for(i=0,i<#portres.input,i++){\n"
       			+"if(portres.input[i].name.name==portRequest.name.name){\n"
       			+"var<<portres.input[i]\n"
       			+"}\n"
       			+"};\n"
                        +"iniRequestlocations=\"locations.ini\";\n"
                        +"parseIniFile@IniUtils(iniRequestlocations)(iniResponselocations);\n"
                        +"config.location=iniResponse.fileParameter.port_location;\n"
                        +"filecontent=\"[locations] \\n"+inputPortData+"=local\";\n"
                        +"locationsfile.content=filecontent;\n"
                        +"locationsfile.filename=\"locations.ini\";\n"
                        +"writeFile@File(locationsfile)();\n"
